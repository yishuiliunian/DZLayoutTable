
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// DZLayoutTable
#define COCOAPODS_POD_AVAILABLE_DZLayoutTable
#define COCOAPODS_VERSION_MAJOR_DZLayoutTable 0
#define COCOAPODS_VERSION_MINOR_DZLayoutTable 1
#define COCOAPODS_VERSION_PATCH_DZLayoutTable 0

// FBSnapshotTestCase
#define COCOAPODS_POD_AVAILABLE_FBSnapshotTestCase
#define COCOAPODS_VERSION_MAJOR_FBSnapshotTestCase 1
#define COCOAPODS_VERSION_MINOR_FBSnapshotTestCase 6
#define COCOAPODS_VERSION_PATCH_FBSnapshotTestCase 0

